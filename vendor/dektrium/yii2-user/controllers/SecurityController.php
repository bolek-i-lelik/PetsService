...
class SecurityController extends Controller
{
    ...
    public function authenticate(ClientInterface $client)
    {
        ...
        if (!$this->module->enableRegistration && ($account === null || $account->user === null)) {
            \Yii::$app->session->setFlash('danger', \Yii::t('user', 'Регистрация временно не возможна. Приносим свои извинения за временные неудобства.'));
        ...
        if ($account->user instanceof User) {
            if ($account->user->isBlocked) {
                \Yii::$app->session->setFlash('danger', \Yii::t('user', 'Ваш аккаунт заблокирован.'));
                $this->action->successUrl = Url::to(['/user/security/login']);
            } else {
